...
/// Controls tab selection from outside [TabContainerPage].
/// [TabContainerPageController.length] must equal the length of [children] and [tabs].
class TabContainerPageController extends ValueNotifier<int> {
  int _index;
  int _prevIndex;
  final int _length;

  TabContainerPageController({
    int initialIndex = 0,
    required int length,
  })  : assert(length > 0),
...
///
/// Handles styling and animation and exposes control over tab selection through [TabContainerPageController].
class TabContainerPage extends ImplicitlyAnimatedWidget {
  const TabContainerPage({
    Key? key,
  ...
  final TabContainerPageController? controller;
  ...
  @override
  _TabContainerPageState createState() => _TabContainerPageState();
}

class _TabContainerPageState extends AnimatedWidgetBaseState<TabContainerPage> {
  TabContainerPageController? _controller;
                ...
                children: [
                  ...
                  widget.tabs is! List<String>
                      ...
                      : Text(widget.tabs[i],
                          ...
                          style: style),
                  style.color == launchPageTopBtnBg() ||
                          style.color == launchPageTopBtnBgs()
                      ? Container(
                          width: HYSizeFit.setRpx(21),
                          height: HYSizeFit.setRpx(4),
                          margin: EdgeInsets.only(top: HYSizeFit.setRpx(5)),
                  ...
                  // widget.tabs is! List<String>
                  //     ? widget.tabs[i]
                  //     : Text(widget.tabsValues[i].toString(),
                  //         textAlign: TextAlign.center,
                  //         overflow: TextOverflow.fade,
                  //         textDirection: _textDirection,
                  //         style: style.copyWith(
                  //           fontWeight: FontWeight.bold,
                  //           fontSize: HYSizeFit.setRpx(27),
                  //           fontFamily: 'Heavy',
                  //         )),
                  SizedBox(
  ...
  void initState() {
    _controller = widget.controller ??
        TabContainerPageController(length: widget.tabs.length);
...
class RenderTabFrame extends RenderBox
  ...
  RenderTabFrame({
    ...
    required TabContainerPageController controller,
  ...
  set controller(TabContainerPageController value) {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final Path horizontalPath = Path()
      ...
      //TODO:右侧无圆角
      ..lineTo(width, height - _tabExtent - 0)
      ..quadraticBezierTo(
        width,
        height - _tabExtent,
        max(width - 0, rightPos),
        height - _tabExtent,
      )
      //TODO：右侧最后一个圆角
      ..lineTo(
          min(
              max(width - 0, rightPos),
              // min(max(width - _radius, rightPos),
              min(width, rightPos + _radius - 12)),
          height - _tabExtent)
      //TODO：右下脚圆角
      ..quadraticBezierTo(
        rightPos,
      ...
      //TODO：左下角圆角
      ..lineTo(leftPos, height - _tabExtent + _radius - 12)
      ..quadraticBezierTo(
        leftPos,
        height - _tabExtent,
        //TODO:左下脚第一个圆角
        max(min(0, leftPos), max(0, leftPos - _radius + 12)),
        // max(min(_radius, leftPos), max(0, leftPos - _radius + 12)),
        height - _tabExtent,
      )
      //TODO:左侧无圆角
      ..lineTo(min(0, leftPos), height - _tabExtent)
